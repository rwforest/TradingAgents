...
The trading agent is built on the LangGraph framework, which allows for the creation of cyclical, stateful agent graphs. The core of the application is the `TradingAgentsGraph`, which orchestrates the entire workflow.
...
- **`GraphSetup`**: This class is responsible for constructing the LangGraph workflow. It defines the nodes (agents) and edges (transitions) of the graph. The graph is a `StateGraph`, where each node modifies a shared `AgentState`.
...
1. **Initiation**: The workflow begins when the `propagate` method of `TradingAgentsGraph` is called with a company ticker and a trade date.

2. **Analyst Sequence**: The initial state is passed to a sequence of analyst agents. By default, the sequence is:
   - **Market Analyst**: Performs technical analysis of the stock.
   - **Social Media Analyst**: Analyzes social media sentiment.
   - **News Analyst**: Gathers and analyzes news articles.
   - **Fundamentals Analyst**: Analyzes the company's financial fundamentals.

3. **Investment Debate**: The reports from the analysts are then passed to a debate between a "Bull Researcher" and a "Bear Researcher". They argue for and against investing in the company.
...
4. The report is stored in the `market_report` field of the `AgentState`.
...
1. The Social Media Analyst is tasked with analyzing social media sentiment.
...
4. The report is stored in the `sentiment_report` field of the `AgentState`.
...
1. The News Analyst is tasked with gathering and analyzing news.
...
4. The report is stored in the `news_report` field of the `AgentState`.
...
1. The Fundamentals Analyst is tasked with analyzing the company's financial health.
...
4. The report is stored in the `fundamentals_report` field of the `AgentState`.
...
After the analysts have gathered and presented their reports, the workflow moves into a series of debates and decision-making stages.
...
6. **Next Agent**: The updated `AgentState` is passed to the next agent in the workflow.
