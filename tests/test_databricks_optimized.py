...
# Create a custom config optimized for Databricks context limits
...
# Optimize to reduce context size
...
print(f"  Max Risk Discuss Rounds: {config['max_risk_discuss_rounds']}")
print()

# Initialize with custom config - use only essential analysts to reduce context
print("Initializing TradingAgentsGraph...")
print("Note: Using only 'market' and 'news' analysts to optimize context size")
ta = TradingAgentsGraph(
    debug=True,
    config=config,
    selected_analysts=["market", "news"]  # Reduced from ["market", "social", "news", "fundamentals"]
)
...
try:
    ...
    print(f"**Analysts Used:** market, news (optimized for context)")
    print("\n" + "="*80 + "\n")
